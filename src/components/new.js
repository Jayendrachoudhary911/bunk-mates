...
import {
  LocationOn, AccessTime, CheckCircle, Cancel, Edit, Add, ContentCopy, Settings as SettingsIcon, Info as InfoIcon,
...
const revealSurpriseEvent = async (eventId) => {
  ...
  const updatePermissions = async (newPermissions) => {
    try {
      await updateDoc(doc(db, "trips", id), { permissions: newPermissions });
      setTripPermissions(newPermissions);
  ...
  const promoteToAdmin = async (uid) => {
    if (!tripAdmins.includes(currentUseruid)) return;
    const newAdmins = [...tripAdmins, uid];
    try {
      await updateDoc(doc(db, "trips", id), { admins: newAdmins });
      setTripAdmins(newAdmins);
      setSnackbar({ open: true, message: "Member promoted to admin!" });
    } catch (error) {
      console.error("Error promoting member:", error);
      setSnackbar({ open: true, message: "Failed to promote member." });
    }
  };

  // Function to demote admin
  const demoteAdmin = async (uid) => {
    if (uid === trip?.createdBy) return; // Cannot demote creator
    const newAdmins = tripAdmins.filter(a => a !== uid);
    try {
      await updateDoc(doc(db, "trips", id), { admins: newAdmins });
      setTripAdmins(newAdmins);
      setSnackbar({ open: true, message: "Admin demoted!" });
    } catch (error) {
      console.error("Error demoting admin:", error);
      setSnackbar({ open: true, message: "Failed to demote admin." });
    }
  };
...
useEffect(() => {
  ...
  const unsubscribeTrip = onSnapshot(tripDocRef, (docSnap) => {
    if (docSnap.exists()) {
      ...
      setTripPermissions(tripData.permissions || {
        canAddMembers: 'all',
        canAddExpenses: 'all',
        canAddChecklists: 'all',
        canAddTimelines: 'all',
      ...
      setTripAdmins(tripData.admins || [tripData.createdBy]);
...
const updateExpense = async () => {
  if (!editingExpense || !newExpense.name || !newExpense.amount || !newExpense.date || !newExpense.time) {
    setSnackbar({ open: true, message: "Please fill all required fields." });
    return;
  }
  ...
  try {
    ...
    const updatedExpenses = expenses.map((exp) =>
      exp.dateTime === editingExpense.dateTime ? updatedExpense : exp
    );
    ...
    setBudget((prev) => ({
      ...prev,
      used: updatedExpenses.reduce((sum, e) => sum + (Number(e.amount) || 0), 0),
      expenses: updatedExpenses,
    }));
...
{visibleExpenses.map((exp, idx) => {
  const canEdit = exp.payers?.some(p => p.uid === currentUseruid) || exp.paidBy === currentUseruid;
  return (
    <Box
      key={idx}
      sx={{
            ...
            onClick={() => {
              ...
              setExpenseContributors(
                memberDetails.map(member => ({
                  ...
                  name: member.name || 'Unknown',
                  photoURL: member.photoURL,
                  included: exp.payers?.some(p => p.uid === member.uid) || exp.paidBy === member.uid,
                  paidAmount: exp.payers?.find(p => p.uid === member.uid)?.amount || (exp.paidBy === member.uid ? exp.amount : 0),
                }))
              );
...
>
  ...
  >
    <Typography variant="h6" fontWeight="bold">
      {tripAdmins.includes(currentUseruid) ? "Trip Settings" : "Trip Info"}
    </Typography>
  ...
  <Box sx={{ mb: 3 }}>
    ...
    <Typography variant="body1" sx={{ mb: 2 }}>
      {trip?.from && trip?.to ? `${trip.from} → ${trip.to}` : "Not set"}
    </Typography>
    ...
    <Typography variant="body1" sx={{ mb: 2 }}>
      {getMemberName(trip?.createdBy) || "N/A"}
    </Typography>
  ...
  {/* Settings for Admins */}
  {tripAdmins.includes(currentUseruid) && (
    <Box sx={{ mt: 3 }}>
      <Divider sx={{ mb: 2 }} />
      ...
      {Object.keys(tripPermissions).map((perm) => (
        <Box key={perm} sx={{ mb: 2 }}>
          <Typography variant="subtitle2" sx={{ mb: 1 }}>
            {perm.replace('canAdd', 'Can Add ').replace('canEdit', 'Can Edit ')}:
          </Typography>
          <Button
            variant={tripPermissions[perm] === 'all' ? 'contained' : 'outlined'}
            onClick={() => updatePermissions({ ...tripPermissions, [perm]: tripPermissions[perm] === 'all' ? 'admins' : 'all' })}
            sx={{ mr: 1 }}
          >
            All Members
          </Button>
          <Button
            variant={tripPermissions[perm] === 'admins' ? 'contained' : 'outlined'}
            onClick={() => updatePermissions({ ...tripPermissions, [perm]: 'admins' })}
          >
            Admins Only
          </Button>
        </Box>
      ...
      {memberDetails.map((user) => (
        <Box key={user.uid} display="flex" alignItems="center" justifyContent="space-between" sx={{ mb: 1 }}>
          <Typography>{getMemberName(user.uid)}</Typography>
          {tripAdmins.includes(user.uid) ? (
            user.uid !== trip?.createdBy && (
              <Button size="small" onClick={() => demoteAdmin(user.uid)}>Demote</Button>
            )
          ) : (
            <Button size="small" onClick={() => promoteToAdmin(user.uid)}>Promote to Admin</Button>
          )}
        </Box>
      ))}
      ...
      <Typography variant="subtitle2" sx={{ mb: 1, mt: 2 }}>Accent:</Typography>
      {['blue', 'green', 'purple', 'orange'].map((acc) => (
        <Button
          key={acc}
          variant={accent === acc ? 'contained' : 'outlined'}
      ...
      <Button
        ...
        startIcon={<DeleteOutlineIcon />}
        color="error"
        sx={{ borderRadius: 3, backgroundColor: "#ff000010" }}
        onClick={() => {
