...
function BudgetGridCard({
  ...
  history,
  getCurrentBalance,
  getCurrentBalanceing,
  CATEGORY_ICONS,
  buttonWeatherBg,
  isDark
}) {
  const balance = getCurrentBalance(item);
  const balanceing = getCurrentBalanceing(item);
...
useEffect(() => {
  const fetchContributors = async () => {
    if (!item.contributors?.length) return;

    try {
      const users = await Promise.all(
        item.contributors.map(async (contributor) => {
          if (!contributor.uid) {
            return contributor;
          }
    ...
    {isOver && (
      <Box
        sx={{
          ...
          background:
            "linear-gradient(135deg, rgba(255,80,80,0.14), rgba(255,80,80,0.08))",
          ...
          boxShadow:
            "0 4px 14px rgba(255,80,80,0.12)",
        }}
      >
        <Typography
          sx={{
        ...
        sx={{
          ...
          background:
            CATEGORY_ICONS[item.category]
              ...
              ? `linear-gradient(
                  135deg,
                  ${CATEGORY_ICONS[item.category]?.mcolor},
                  ${CATEGORY_ICONS[item.category]?.mcolor}CC
                )`
...
function BudgetListCard({
  ...
  const isOver =
    Number(balanceing) > Number(item.amount);
  ...
  return (
    <Box
      ...
      onClick={() =>
        history(
          `/budget-mngr?index=${itemIndex}&expdrawer=true`
        )
      }
      sx={{
        width: "100%",
...
const handleOpenExpDrawer = (index) => {
  ...
  const getCurrentBalance = (item) => {
    ...
    const getCurrentBalanceing = (item) => {
    const totalExpenses = item.expenses?.reduce((sum, e) => sum + e.amount, 0) || 0;
          ...
          {filteredBudgets.map((item, idx) => {
            ...
            return (
              <BudgetGridCard
                ...
                getCurrentBalanceing={getCurrentBalanceing}
