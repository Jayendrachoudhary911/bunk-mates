...
// In the fetchReminders function
const fetchReminders = async (currentUser) => {
  ...
  setLoading(true);
  try {
    // ✅ FIXED: Query now filters by UID on the server
    const q = query(
      collection(db, "reminders"),
      where("uid", "==", currentUser.uid), // Filter by user ID
...
const handleAddReminder = async () => {
  ...
  try {
    ...
    // ✅ FIXED: Update state locally instead of refetching
